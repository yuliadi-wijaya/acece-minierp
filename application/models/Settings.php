...
class Settings extends CI_Model {
    ...
    public function bank_update_by_id($bank_id) {
        ...
        $data = array('bank_name' => $this->input->post('bank_name', TRUE), 'ac_name' => $this->input->post('ac_name', TRUE), 'ac_number' => $this->input->post('ac_no', TRUE), 'branch' => $this->input->post('branch', TRUE), 'signature_pic' => (!empty($signature_pic) ? $signature_pic : $old_logo), 'status' => 1);
        $bank_coaupdate = ['HeadName' => $this->input->post('bank_name', TRUE) ];
    ...
    public function customer_info() {
        $query = $this->db->select('*')->from('customer_information')->join('invoice', 'invoice.customer_id = customer_information.customer_id')->group_by('invoice.customer_id')->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
        return false;
    ...
    public function product_info() {
        ...
        $query = $this->db->select('
                                ...
                                ')->from('invoice_details')->join('product_information', 'invoice_details.product_id = product_information.product_id')->join('invoice', 'invoice.invoice_id = invoice_details.invoice_id')->where('invoice.customer_id', $customer_id)->where('invoice.date >=', $from)->where('invoice.date <=', $to)->where('invoice_details.product_id', $product_id)->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
        return false;
    ...
    public function password_recovery($data = array()) {
        return $this->db->select("*")->from('user_login')->where('username', $data['email'])->get();
    }
    ...
    public function token_matching($token_id) {
        return $this->db->select("*")->from('user_login')->where('security_code', $token_id)->get();
    }
    // Currency list
